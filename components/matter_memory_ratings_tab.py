...
def matter_memory_ratings_tab(username: str, timepoint_id="T_01", timepoint_modifier="T01"):
    st.markdown(f"<h1>{timepoint_modifier} Matter Memory Ratings</h1>", unsafe_allow_html=True)
    user_supabase = get_user_supabase()
    bucket = user_supabase.storage.from_("data")
    
    # Check if matter data exists
    matter_file = build_supabase_path(username, timepoint_id, "matter2.csv")
    ...
    # Check if form submission confirmation exists (same file as matter overview)
    ...
    if matter_data_exists:
        ...
        with st.form("matter_memory_ratings_confirmation"):
            st.markdown("**I have completed the memory data upload.**")
            submitted = st.form_submit_button("Submit Confirmation")
            
            if submitted:
                # Create a confirmation file in the database (same file as matter overview)
